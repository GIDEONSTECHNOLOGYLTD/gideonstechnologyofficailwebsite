...
return function (App $app, ContainerInterface $container) {
    ...
    $app->group('/admin', function (RouteCollectorProxy $group) use ($container) {
        ...
        $group->group('/config', function (RouteCollectorProxy $group) {
            if (!RouteRegistry::isRegistered('GET', '/admin/config')) {
                RouteRegistry::register('GET', '/admin/config');
                $group->get('', [ConfigController::class, 'index'])->setName('admin.config');
            }
            
            if (!RouteRegistry::isRegistered('POST', '/admin/config/save')) {
                RouteRegistry::register('POST', '/admin/config/save');
                $group->post('/save', [ConfigController::class, 'save'])->setName('admin.config.save');
            }
            
            if (!RouteRegistry::isRegistered('GET', '/admin/config/maintenance')) {
                RouteRegistry::register('GET', '/admin/config/maintenance');
                $group->get('/maintenance', [ConfigController::class, 'maintenance'])->setName('admin.config.maintenance');
            }
            
            if (!RouteRegistry::isRegistered('POST', '/admin/config/maintenance/update')) {
                RouteRegistry::register('POST', '/admin/config/maintenance/update');
                $group->post('/maintenance/update', [ConfigController::class, 'updateMaintenance'])->setName('admin.config.maintenance.update');
            }
        });
        ...
        $group->group('/payment', function (RouteCollectorProxy $group) {
            if (!RouteRegistry::isRegistered('GET', '/admin/payment')) {
                ...
                $group->get('', [PaymentController::class, 'index'])->setName('admin.payment');
            }
            
            if (!RouteRegistry::isRegistered('POST', '/admin/payment/save')) {
                RouteRegistry::register('POST', '/admin/payment/save');
                $group->post('/save', [PaymentController::class, 'save'])->setName('admin.payment.save');
            }
            
            if (!RouteRegistry::isRegistered('POST', '/admin/payment/test-paypal')) {
                RouteRegistry::register('POST', '/admin/payment/test-paypal');
                $group->post('/test-paypal', [PaymentController::class, 'testPayPal'])->setName('admin.payment.test-paypal');
            }
            
            if (!RouteRegistry::isRegistered('POST', '/admin/payment/test-stripe')) {
                RouteRegistry::register('POST', '/admin/payment/test-stripe');
                $group->post('/test-stripe', [PaymentController::class, 'testStripe'])->setName('admin.payment.test-stripe');
            }
        });
